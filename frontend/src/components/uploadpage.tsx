      ...
      <main className="relative z-10 flex-1 flex flex-col items-center justify-center p-8">
        ...
        >
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="mt-6 flex justify-center gap-4">
            <Button onClick={handleAnalyze} disabled={!file || uploading} className="relative overflow-hidden group bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-600 hover:to-indigo-700 text-white px-6 py-2 rounded-xl font-medium shadow-lg">
              {uploading ? (
            ...
            <Button onClick={handleClear} disabled={!file || uploading} variant="outline" className="bg-white/10 text-white hover:bg-white/20 border-purple-300/50 flex items-center">
...
{result && (
  ...
  >
    <div className="text-xl font-small mt-2">
      ...
      <p className={`${result.vgg_prediction === "Authentic Image ✅" ? "text-green-400" : "text-red-400"}`}>
        VGG Prediction: {result.vgg_prediction}
      </p>
      <p className={`${result.match_ratio === "Authentic Image ✅" ? "text-green-400" : "text-red-400"}`}>
        Key Point Approach: {result.match_ratio}
      </p>
    </div>
  </motion.div>
)}
